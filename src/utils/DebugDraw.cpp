...
void btPlaneSpace1(const T& n, T& p, T& q) {
	if (fabsf(n[2]) > 0.7071067811865475244008443621048490f) {
		...
		render::f32 a = n[1] * n[1] + n[2] * n[2];
		render::f32 k = 1.0f / sqrtf(a);
		p[0] = 0;
		p[1] = -n[2] * k;
		p[2] = n[1] * k;
		// set q = n x p
...
namespace render {
    namespace utils {
        ...
        void IDebugDrawer::sphere(f32 radius, const mat4f& transform, const vec4f& color) {
        ...
        void IDebugDrawer::transform(const mat4f& transform, f32 orthoLen) {
            vec3f start = transform.w;
            line(start, start + transform.basis() * vec3f(orthoLen, 0, 0), vec4f(f32(1.), f32(0.3), f32(0.3), 1.0f));
            line(start, start + transform.basis() * vec3f(0, orthoLen, 0), vec4f(f32(0.3), f32(1.), f32(0.3), 1.0f));
            line(start, start + transform.basis() * vec3f(0, 0, orthoLen), vec4f(f32(0.3), f32(0.3), f32(1.), 1.0f));
        }
        ...
        void IDebugDrawer::arc(
            ...
            if (!nSteps) nSteps = 1;
            vec3f prev = center + vx * radiusA * cosf(minAngle) + vy * radiusB * sinf(minAngle);
            if (drawSect)
            {
            ...
            for (int i = 1; i <= nSteps; i++)
            {
                ...
                vec3f next = center + vx * radiusA * cosf(angle) + vy * radiusB * sinf(angle);
        ...
        void IDebugDrawer::spherePatch(
            ...
            for (int i = 0; i < n_hor; i++)
            {
                ...
                for (int j = 0; j < n_vert; j++)
                {
                    ...
                    f32 sps = sinf(psi);
                    f32 cps = cosf(psi);
                    pvB[j] = center + iv * cth * cps + jv * cth * sps + kv * sth;
                    if (i)
                    {
        ...
        void IDebugDrawer::box(const vec3f& min, const vec3f& max, const mat4f& transform, const vec4f& color) {
        ...
        void IDebugDrawer::capsule(f32 radius, f32 halfHeight, Axis upAxis, const mat4f& transform, const vec4f& color) {
            int stepDegrees = 30;
            ...
            // Draw the ends
            {
                mat4f childTransform = transform;
                childTransform.w = transform * capStart;
                {
                    vec3f center = childTransform.w;
                    vec3f up = childTransform.basis()[(upAxis + 1) % 3];
                    vec3f axis = -childTransform.basis()[upAxis];
                    f32 minTh = -HALF_PI;
                    f32 maxTh = HALF_PI;
                    f32 minPs = -HALF_PI;
                    f32 maxPs = HALF_PI;
        ...
        void IDebugDrawer::cylinder(f32 radius, f32 halfHeight, Axis upAxis, const mat4f& transform, const vec4f& color) {
            vec3f start = transform.w;
            ...
            for (int i = 0; i < 360; i += stepDegrees)
            {
                capEnd[(upAxis + 1) % 3] = capStart[(upAxis + 1) % 3] = sinf(f32(i) * RADS_PER_DEG) * radius;
                capEnd[(upAxis + 2) % 3] = capStart[(upAxis + 2) % 3] = cosf(f32(i) * RADS_PER_DEG) * radius;
                line(start + transform.basis() * capStart, start + transform.basis() * capEnd, color);
            }
            ...
            arc(start - transform.basis() * (offsetHeight), transform.basis() * yaxis, transform.basis() * xaxis, radius, radius, 0, PI2, color, false, f32(10.0));
            arc(start + transform.basis() * (offsetHeight), transform.basis() * yaxis, transform.basis() * xaxis, radius, radius, 0, PI2, color, false, f32(10.0));
        }

        void IDebugDrawer::cone(f32 radius, f32 height, Axis upAxis, const mat4f& transform, const vec4f& color) {
            int stepDegrees = 30;
            vec3f start = transform.w;
            ...
            for (int i = 0; i < 360; i += stepDegrees)
            {
                ...
                line(start + transform.basis() * (offsetHeight), start + transform.basis() * capEnd, color);
            }

            line(start + transform.basis() * (offsetHeight), start + transform.basis() * (-offsetHeight + offsetRadius), color);
            line(start + transform.basis() * (offsetHeight), start + transform.basis() * (-offsetHeight - offsetRadius), color);
            line(start + transform.basis() * (offsetHeight), start + transform.basis() * (-offsetHeight + offset2Radius), color);
            line(start + transform.basis() * (offsetHeight), start + transform.basis() * (-offsetHeight - offset2Radius), color);
            ...
            vec3f xaxis(0, 0, 0);
            xaxis[(upAxis + 1) % 3] = f32(1.0);
            arc(start - transform.basis() * (offsetHeight), transform.basis() * yaxis, transform.basis() * xaxis, radius, radius, 0, PI2, color, false, 10.0);
        }

        void IDebugDrawer::plane(const vec3f& normal, f32 planeConst, const mat4f& transform, const vec4f& color) {
        ...
        void DebugDraw::sphere(f32 radius, const mat4f& transform, const vec4f& color) {
            if (!DebugDraw::drawer) return;
        ...
        void DebugDraw::transform(const mat4f& transform, f32 orthoLen) {
            if (!DebugDraw::drawer) return;
        ...
        void DebugDraw::box(const vec3f& min, const vec3f& max, const mat4f& transform, const vec4f& color) {
            if (!DebugDraw::drawer) return;
            DebugDraw::drawer->box(min, max, transform, color);
        }

        void DebugDraw::capsule(f32 radius, f32 halfHeight, Axis upAxis, const mat4f& transform, const vec4f& color) {
            if (!DebugDraw::drawer) return;
            DebugDraw::drawer->capsule(radius, halfHeight, upAxis, transform, color);
        }

        void DebugDraw::cylinder(f32 radius, f32 halfHeight, Axis upAxis, const mat4f& transform, const vec4f& color) {
            if (!DebugDraw::drawer) return;
            DebugDraw::drawer->cylinder(radius, halfHeight, upAxis, transform, color);
        }

        void DebugDraw::cone(f32 radius, f32 height, Axis upAxis, const mat4f& transform, const vec4f& color) {
            if (!DebugDraw::drawer) return;
            DebugDraw::drawer->cone(radius, height, upAxis, transform, color);
        }

        void DebugDraw::plane(const vec3f& normal, f32 planeConst, const mat4f& transform, const vec4f& color) {
            if (!DebugDraw::drawer) return;
